...
- **Per-OS process control is split by build-tag filename** (`process_unix.go` / `process_windows.go`), the same pattern `backend/src/core/security/sandbox/` already uses for the same reason. POSIX: a process group, `SIGTERM` then `SIGKILL`. Windows: a process group plus `CTRL_BREAK_EVENT` for a graceful stop — the same technique `core/tools/host_runtime.py` already uses to interrupt one execution, applied here to a whole child process — falling back to `taskkill /T /F` to reap the tree if that doesn't land within the grace period.
