...
- **The CLI ships now**: `python backend/main.py skills add|list|update|remove|token`, over the same functions the routes use. An optional positional and a subparser cannot coexist in one argparse parser, so the first word selects between two parsers — `python backend/main.py data.csv` is unchanged. Milestone 8's binary will front these rather than reimplement the fetch.
