...
- **Per-session runtime isolation:** Generated code never runs in the API process. The default `EXECUTION_BACKEND=host` gives each session its own subprocess, with a memory ceiling, a per-step timeout and an interrupt — which contains runaway code but is **not** a security boundary: it runs as you, with your files. `EXECUTION_BACKEND=docker` gives each session a restricted-permission container instead, dropping capabilities and stripping access to host I/O, and remains the right answer for data or questions you did not write yourself. Which one is in force is reported on `/settings` rather than assumed.
