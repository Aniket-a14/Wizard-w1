...
class DataAnalysisAgent:
    ...
    def run(self, instruction: str, df: pd.DataFrame) -> Tuple[str, str, Optional[str]]:
        ...
        if not code and settings.MODEL_TYPE in ['local', 'hybrid']:
            ...
            if local_gen:
                ...
                try:
                    response = local_gen(prompt, max_new_tokens=100, pad_token_id=50256)[0]['generated_text']
    ...
    def _process_code(self, code: str, df: pd.DataFrame) -> Tuple[str, str, Optional[str]]:
    ...
    def _execute_safe(self, code: str, df: pd.DataFrame) -> Tuple[str, str, Optional[str]]:
        ...
        # Globals
        exec_globals = {
            "np": np, "pd": pd, "sns": sns, "plt": plt, "df": df,
            "stats": StatisticalToolkit,
            "__builtins__": {k: v for k, v in builtins.__dict__.items() if k not in ['eval', 'exec', 'open', 'exit', 'quit']}
        }
            ...
            if is_plotting:
                ...
                try:
                    ...
                    if len(tree.body) == 1 and isinstance(tree.body[0], ast.Expr):
                        result = eval(compile(tree, filename="<string>", mode="eval"), exec_globals)
                ...
                return output.strip() if output else "Executed successfully.", code, None
