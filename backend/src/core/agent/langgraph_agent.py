...
from src.core.prompts import create_planning_prompt, create_prompt, create_replan_prompt
...
class WorkflowState:
    ...
    def __init__(self, instruction: str, df: pd.DataFrame, mode: str = "standard", catalog: dict | None = None):
...
class LangGraphAgent:
    ...
    def step_plan(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            state.plan = (
                f"1. Analyze dataset columns: {state.df.columns.tolist()}\n2. Complete request: {state.instruction}"
            )
            state.status = "executing"
...
Write python code ONLY for this step. Do not rewrite prior steps, just continue. Keep imports minimal."""
        ...
        neg_examples = self._get_negative_examples(
            state.instruction, state.df.columns.tolist() if state.df is not None else None
        )
    ...
    def step_execute_code(self, state: WorkflowState, on_stdout: Callable[[str], None] | None = None) -> WorkflowState:
        """
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name.split(".")[0] in banned_modules:
                            logger.warning(
                                "AST Guardrail blocked code execution", reason=f"Banned import: {alias.name}"
                            )
                    ...
                    if node.module and node.module.split(".")[0] in banned_modules:
                        logger.warning(
                            "AST Guardrail blocked code execution", reason=f"Banned import from: {node.module}"
                        )
                    ...
                    if isinstance(node.func, ast.Name):
                        ...
                        if func_name == "open" and node.args:
                            ...
                            if isinstance(path_node, ast.Constant) and isinstance(path_node.value, str):
                                ...
                                if not (
                                    clean_path.startswith("/workspace")
                                    or clean_path.startswith("/app/workspace")
                                    or clean_path.startswith("/app/backend/workspace")
                                ):
                                    logger.warning(
                                        "AST Security block: File path traversal breakout attempt", path=path_val
                                    )
    ...
    def _get_negative_examples(self, query: str, columns: list[str] | None = None) -> str:
        """
    ...
    def step_vision_explain(self, base64_image: str) -> str:
        ...
        try:
            ...
            vision_llm = ChatOllama(model=vision_model_name, base_url=settings.OLLAMA_BASE_URL, temperature=0.2)
            ...
            message = HumanMessage(
                content=[
                    {
                        "type": "text",
                        "text": "Describe this data visualization chart in 2-3 sentences. Explain the visible trend, the axes, and any key insights.",
                    },
