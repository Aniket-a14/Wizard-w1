...
class ReportingEngine:
    ...
    def generate_executive_summary(timespan_seconds: int = 3600) -> str:
        ...
        """
        now = time.time()
        recent_memories = [m for m in working_memory.memories if (now - m["timestamp"]) < timespan_seconds]

        if not recent_memories:
            return "No recent analysis history found to summarize."

        report = "# 📊 Wizard AI: Executive Data Story\n"
        ...
        report += "## 🛡️ Reliability & Governance\n"
        ...
        avg_score = sum(entry.get("meta", {}).get("quality_score", 100) for entry in recent_memories) / len(
            recent_memories
        )
