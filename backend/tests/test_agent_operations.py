...
def mock_agent_execute():
    # Patch the real Agent.run to return specific mocked responses
    # because real LLM calls are slow and require models to be running.
    with patch('src.core.agent.agent.DataAnalysisAgent.run') as mock:
...
def test_basic_operations(tips_df, query, mock_agent_execute):
    ...
    # Note: The fixture mocks the method on the class/instance, so calling the instance method hits the mock
    result, code, image = agent.run(query, tips_df)
