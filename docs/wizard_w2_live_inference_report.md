...
**Recommendation for the application itself** (not just the benchmark): either don't cache code that resolves an absolute path via `workspace_path()`, or re-resolve such paths against the replaying session at cache-hit time; and consider whether the cache should be scoped by provider/model at all, given how easily two genuinely different configurations end up silently sharing one one's output.
...
- **Two real application bugs were found and fixed this session, not just measured.** The `data_mode` pass-through bug ([orchestrator.py:1669](../backend/src/core/agent/orchestrator.py#L1669)) meant hybrid and cloud-only were completely non-functional on a real install before today. The semantic cache and trajectory-memory system's missing session/provider scoping (findings 2, 2b, 2c) meant a live comparison across configurations silently borrowed each other's work — not a benchmark artifact, a standing correctness gap that would affect two real users on the same install today.
