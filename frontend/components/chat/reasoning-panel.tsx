  ...
  return (
    <div
      className={cn(
        ...
        streaming ? "border-brand/30" : "border-border",
      )}
    >
      <button
        type="button"
        ...
        aria-expanded={open}
      >
        <Brain className={cn("h-3.5 w-3.5 shrink-0", streaming && "animate-pulse text-brand")} />
        <span>{streaming ? "Thinking…" : seconds ? `Thought for ${seconds}s` : "Reasoning"}</span>
        <ChevronRight
          className={cn(
      ...
      {open && (
        <div
          ref={bodyRef}
          className="max-h-64 overflow-y-auto border-t border-border px-3.5 py-3 text-[13px] leading-relaxed text-muted-foreground"
        >
          <p className="whitespace-pre-wrap break-words">{content}</p>
          {streaming && <span className="caret !h-[0.9em]" aria-hidden="true" />}
        </div>
      )}
    </div>
  )
